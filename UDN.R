...
UDNsearch(keyword, DateB, DateE, news = news, page=1) %>% cat()

#執行UDNcrawler 
labor_97 <- UDNcrawler()
save(labor_97, file = "D:/labor_97.rda") #存檔路徑用桌面會無法寫入

#關機
shutdown()

#單一頁面
url <- UDNsearch(keyword, DateB, DateE, news = news, page=1)
doc <- read_html(url, encoding = )
...
xml_find_all(doc, xpath = "//span[@class='date']") %>% xml_text() %>% iconv(from = "UTF-8", to = "UTF-8") -> date